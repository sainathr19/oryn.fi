...
import { convertBigIntToUSD, calculateMaxBorrowPower } from "../utils/borrowCalculations";
...
export const Dashboard = () => {
  ...
  useEffect(() => {
    if (userPositions && Array.isArray(userPositions)) {
      ...
      const validPositions = userPositions.filter((position: any) => 
        position && 
  ...
  const getMaxMintAmount = (position: PositionDetails): number => {
    if (!position || !position.collateralValueUSD || !position.debtValueUSD) return 0;
    ...
    return (
      <div className="flex items-center justify-center h-screen">
        <div className="text-center">
          ...
          <p className="text-gray-600">Connect your wallet to view your positions</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto">
        ...
        {positions.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500">You don't have any deposited NFT positions yet.</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            {/* Positions List */}
            <div className="lg:col-span-2">
              <h2 className="text-xl font-semibold text-gray-800 mb-4">Your Positions</h2>
              <div className="space-y-4">
                {positions.map((position) => (
                  <div
                    key={position.positionId ? position.positionId.toString() : Math.random().toString()}
                    ...
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <h3 className="text-lg font-semibold text-gray-800">
                          Position #{position.positionId ? position.positionId.toString() : 'N/A'}
                        </h3>
                        <p className="text-sm text-gray-600">
                          NFT ID: #{position.uniTokenId ? position.uniTokenId.toString() : 'N/A'}
                        </p>
                      </div>
                      <div className="text-right">
                        <div className={`text-lg font-semibold ${getHealthFactorColor(position.healthFactor)}`}>
                          HF: {position.healthFactor ? (Number(position.healthFactor) / Math.pow(10, 18)).toFixed(2) : 'N/A'}
                        </div>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <div className="text-xs text-gray-500 mt-1">
                            Max: {getMaxMintAmount(selectedPosition).toFixed(2)} OUSDC
                          </div>
                        </div>
                        ...
                        {mintSuccess && (
                          <div className="text-green-600 text-sm bg-green-50 p-2 rounded-lg">
                            ✅ Mint successful! Transaction: {lastMintTxHash?.slice(0, 10)}...
                          </div>
                        )}
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <input
                            ...
                            placeholder="0.00"
                            className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-purple-500"
                            max={convertBigIntToUSD(selectedPosition.debtValueUSD)}
                            step="0.01"
                          />
                          <div className="text-xs text-gray-500 mt-1">
                            Max: {formatUSDValue(selectedPosition.debtValueUSD)} OUSDC
                          </div>
                        </div>
                        ...
                        {repaySuccess && (
                          <div className="text-green-600 text-sm bg-green-50 p-2 rounded-lg">
                            ✅ Repay successful! Transaction: {lastRepayTxHash?.slice(0, 10)}...
                          </div>
                        )}
                    ...
                    <div className="mt-6 pt-6 border-t border-gray-200">
                      ...
                      <div className="space-y-2 text-sm">
                        <div className="flex justify-between">
                          ...
                          <span className="font-medium">{formatUSDValue(selectedPosition.collateralValueUSD)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Current Debt:</span>
                          <span className="font-medium">{formatUSDValue(selectedPosition.debtValueUSD)}</span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Health Factor:</span>
                          <span className={`font-medium ${getHealthFactorColor(selectedPosition.healthFactor)}`}>
                            {(Number(selectedPosition.healthFactor) / Math.pow(10, 18)).toFixed(2)}
                          </span>
                        </div>
                        <div className="flex justify-between">
                          <span className="text-gray-600">Max Borrow:</span>
                          <span className="font-medium">
                            {getMaxMintAmount(selectedPosition).toFixed(2)} OUSDC
                          </span>
                        </div>
                      </div>
                    </div>
