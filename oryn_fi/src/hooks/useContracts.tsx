...
export const useContracts = () => {
  ...
  const [orynUSDContractAddress, setOrynUSDContractAddress] = useState<
    `0x${string}` | null
  >(null);
  ...
  const [lastDepositTxHash, setLastDepositTxHash] = useState<string | null>(
    null
  );
  ...
  const [lastApprovalTxHash, setLastApprovalTxHash] = useState<string | null>(
    null
  );
  ...
  const [tokenApprovalError, setTokenApprovalError] = useState<Error | null>(
    null
  );
  ...
  const fetchUserPositions = useCallback(async () => {
    ...
    try {
      ...
      const positionIds = (await orynContract.read.getUserPositions([
      ...
      const result = (await orynContract.read.getUserPositionDetails([
        address as `0x${string}`,
      ])) as any[];
  ...
  const fetchUserNFTs = useCallback(async () => {
    ...
    try {
      ...
      const balance = (await positionManagerContract.read.balanceOf([
        address as `0x${string}`,
      ])) as bigint;
      ...
      for (let i = 0; i < Number(balance); i++) {
        try {
          const tokenId =
            (await positionManagerContract.read.tokenOfOwnerByIndex([
              address as `0x${string}`,
              BigInt(i),
            ])) as bigint;
  ...
  const fetchNFTPositionDetails = useCallback(
    async (tokenId: number) => {
      ...
      try {
        const details = (await orynContract.read.getNFTPositionDetails([
          BigInt(tokenId),
        ])) as [string, string, number, bigint, bigint, bigint, bigint];
  ...
  const fetchAllNFTDetails = useCallback(
    async (tokenIds: number[]) => {
      ...
      try {
        ...
        const { fetchTokenMetadata, findTokenByAddress } = await import(
          "../utils/tokenMetadata"
        );
        ...
        const detailPromises = tokenIds.map(async (tokenId) => {
          try {
            ...
            if (positionDetails) {
              console.log(
                `Position details for token ${tokenId}:`,
                positionDetails
              );

              const token0Metadata = findTokenByAddress(
                positionDetails.token0,
                metadataResponse
              );
              const token1Metadata = findTokenByAddress(
                positionDetails.token1,
                metadataResponse
              );

              console.log(`Token metadata for token ${tokenId}:`, {
              ...
              if (token0Metadata && token1Metadata) {
                ...
                console.log(`Successfully stored details for token ${tokenId}`);
                return {
                  tokenId,
                  positionDetails,
                  token0Metadata,
                ...
                console.warn(`Missing token metadata for token ${tokenId}:`, {
                  token0: positionDetails.token0,
                  token1: positionDetails.token1,
                  token0Found: !!token0Metadata,
                  token1Found: !!token1Metadata,
                });
              }
            } else {
              console.warn(`No position details returned for token ${tokenId}`);
            }
          } catch (err) {
  ...
  const getOrynUSDContractAddress =
    useCallback(async (): Promise<`0x${string}`> => {
      if (!orynContract) {
        throw new Error("Oryn contract not available");
      }

      try {
        const address = (await orynContract.read.getOrynUSDContractAddress(
          []
        )) as `0x${string}`;
  ...
  const approveOrynUSD = useCallback(async (): Promise<string> => {
    if (!orynUSDContractAddress || !address || !walletClient) {
      throw new Error(
        "OrynUSD contract address, wallet address, or wallet client not available"
      );
    }
    ...
    try {
      console.log(
        `Approving OrynUSD tokens for Oryn contract: ${contractAddresses.ORYN_ENGINE_ADDRESS_V3}`
      );
  ...
  const approveNFT = useCallback(
    async (tokenId: number): Promise<string> => {
      ...
      try {
        console.log(
          `Approving NFT token ID: ${tokenId} for Oryn contract: ${contractAddresses.ORYN_ENGINE_ADDRESS_V3}`
        );
  ...
  const depositUniPosition = useCallback(
    async (
      tokenId: number
      ...
      try {
        ...
        const txHash = await orynContract.write.depositUniPosition([
          BigInt(tokenId),
        ]);
  ...
  const approveAndDepositNFT = useCallback(
    async (
      tokenId: number
    ): Promise<{
      positionId: bigint;
      ...
      try {
        console.log(
          `Starting approve and deposit flow for token ID: ${tokenId}`
        );
  ...
  const mintOrynUSD = useCallback(
    async (positionId: number, amount: bigint): Promise<string> => {
      if (!orynContract || !address) {
        throw new Error("Contract or address not available");
      }
      ...
      try {
        ...
        const txHash = await orynContract.write.mintOrynUSD([
          BigInt(positionId),
          amount,
        ]);
  ...
  const burnOrynUSD = useCallback(
    async (
      positionId: number,
      amount: bigint
    ): Promise<{ approvalTxHash?: string; burnTxHash: string }> => {
      if (!orynContract || !address) {
        throw new Error("Contract or address not available");
      }
      ...
      try {
        console.log(
          `Starting repay flow for ${amount} OrynUSD for position ${positionId}`
        );
        ...
        const txHash = await orynContract.write.burnOrynUSD([
          BigInt(positionId),
          amount,
        ]);
  ...
  const redeemUniPosition = useCallback(
    async (positionId: number): Promise<string> => {
      if (!orynContract || !address) {
        throw new Error("Contract or address not available");
      }
      ...
      try {
        ...
        const txHash = await orynContract.write.redeemUniPosition([
          BigInt(positionId),
        ]);
  ...
  const isNFTDeposited = useCallback(
